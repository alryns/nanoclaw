...
- **Per-agent-group delivery mode.** `ncl groups config update --delivery-mode tools-only` makes an agent group deliver only through explicit outbound tool calls (`send_message`, `send_file`, …): envelopes, tool-call-shaped markup and plain prose stay private, a turn someone is waiting on that delivers nothing is corrected once and then answered with a short placeholder, and a provider error is reported with a fixed notice instead of its text. Meant for small or open-weight models that cannot hold the `<message to>` envelope contract. The default, `envelope`, is unchanged. Migration 25 adds `container_configs.delivery_mode` (NULL = envelope). Under tools-only the runner allows one plain `send_message` per destination per turn, attributes each tool send to the request it answers by its `in_reply_to` stamp, and judges a follow-up pushed into a live turn at its own result. See [docs/agent-runner-details.md](docs/agent-runner-details.md#delivery-modes).
