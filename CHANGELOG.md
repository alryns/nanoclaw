...
- **The host sweep's 30-minute idle timeout is configurable via `NANOCLAW_IDLE_TIMEOUT_MS`.** The sweep kills a container whose heartbeat has gone quiet, and the heartbeat is only touched on provider stream events — so a backend slower than hosted inference (a local model decoding a long turn, or doing minutes of prompt processing before its first token) went silent while working normally and was cold-killed mid-turn. Set the variable in ms and restart the host to give a slow backend headroom; it raises both kill paths, the heartbeat timeout and the per-message claim tolerance. Values outside 2 minutes to 24 hours, and unparseable ones, are refused with a warning and fall back to the built-in 30 minutes — an install that sets nothing behaves exactly as before.
