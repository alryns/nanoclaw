...
- **Channel/provider install skills** — copy the relevant module(s) in from the `channels` or `providers` branch, wire imports, install pinned deps (e.g. `/add-discord`, `/add-slack`, `/add-whatsapp`, `/add-opencode`). User-invoked only: every install skill — all `/add-*` plus the Slack companion skills, which is every skill carrying `nc:` fences — declares `disable-model-invocation: true`, so its description stays out of the coding agent's context and you type `/<name>` to run it. Codex reads the same folders through `.agents/skills` and has its own switch, so each install skill also ships `agents/openai.yaml` with `policy.allow_implicit_invocation: false`, which hides it from Codex's prompt until you mention `$<name>` explicitly.
