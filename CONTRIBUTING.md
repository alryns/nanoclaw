...
- `description`: required — for a model-invocable skill Claude reads it to decide when to invoke the skill; an install skill also sets `disable-model-invocation: true`, so it loads only when the user types `/<name>`
