...
import { archiveProviderTranscript, maybeRotateProviderContinuation } from '../provider-contracts/realize.js';
// The execution-policy, inference, MCP, and memory derivations live in
// claude-config.ts: they are the provider-runtime implementations the
// runtime contract declares, and this query path consumes the same
// functions the contract's registration probes exercise.
import {
...
function createPreCompactHook(assistantName?: string): HookCallback {
  return async (input) => {
    ...
    archiveProviderTranscript('claude', preCompact.transcript_path, preCompact.session_id, assistantName, log);
    return {};
  };
}
...
export class ClaudeProvider implements AgentProvider {
  ...
  constructor(options: ProviderOptions = {}) {
    ...
    this.inference = resolveClaudeInference(
      { model: options.model, effort: options.effort, fastMode: options.fastMode },
      process.env,
    );
    ...
    this.env = {
      ...
      ...resolveClaudeMemoryRuntime(undefined, process.env),
    };
  }
  ...
  maybeRotateContinuation(continuation: string): string | null {
    return maybeRotateProviderContinuation('claude', continuation, this.assistantName, log);
  }
