...
The returned paths use the dedicated `mailbox-attachment` mount class. Runtime
validation admits exactly one inbox/outbox pair for the agent, with matching
group scope and fixed modes; it does not widen `group-state` or
`allowlisted-extra` for arbitrary paths.
...
appears at `/workspace/outbox`, delivery removes it only after the durable
marker commits, and an unavailable backing mount fails the spawn. Re-run the
