...
CREATE TABLE container_configs (
  ...
  delivery_mode          TEXT,                            -- envelope | tools-only; NULL = envelope (added by migration 25)
  updated_at             TEXT NOT NULL
);
```

`delivery_mode` picks the container's delivery contract for chat turns: `envelope` (default, `<message to>` blocks in the final text) or `tools-only` (only outbound tool calls deliver; see [agent-runner-details.md](agent-runner-details.md#delivery-modes)). Set via `ncl groups config update --delivery-mode <mode>`; an unreadable value resolves to `envelope` so a bad row never widens delivery.
...
| 25 | `delivery-mode` | `025-delivery-mode.ts` | `container_configs.delivery_mode` — per-agent-group delivery contract (NULL = envelope) |
