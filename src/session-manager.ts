...
export async function destroySessionMailbox(agentGroupId: string, sessionId: string): Promise<void> {
  ...
  const attachments = await mailbox.attachmentMounts(key);
  await mailbox.destroy(key);
  fs.rmSync(attachments.inboxHostPath, { recursive: true, force: true });
  fs.rmSync(attachments.outboxHostPath, { recursive: true, force: true });
...
export async function writeSessionMessage(
  ...
  // The selected mailbox resolves where attachment bytes live. SQLite returns
  // the local session paths; S3 returns paths inside its trusted FUSE mount.
